...
class PathSegment implements IPathSegment {
  constructor(
    public readonly type: PathSegmentType,
...
function isNotHidden(edge: Edge): boolean {
  ...
  if (
    edge.snapshotType === SnapshotEdgeType.Element &&
    edge.to.type === SnapshotNodeType.Native
  ) {
    return false;
  }
 ...
 */
function propagateGrowth(
  oldG: HeapGraph,
  oldGrowth: TwoBitArray,
  newG: HeapGraph,
  newGrowth: TwoBitArray
): void {
  ...
  while (index < queueLength) {
    ...
    if (
      (oldNodeGrowthStatus === GrowthStatus.NEW ||
        oldNodeGrowthStatus === GrowthStatus.GROWING) &&
      oldNode.numProperties() < newNode.numProperties()
    ) {
    ...
    if (newNode.hasChildren) {
      for (const it = newNode.children; it.hasNext(); it.next()) {
        ...
        if (
          oldEdge !== undefined &&
          !visitBits.get(newChildEdge.toIndex) &&
          shouldTraverse(oldEdgeTmp, false) &&
          shouldTraverse(newChildEdge, false)
        ) {
  ...
  public findLeakPaths(log: Log): LeakRoot[] {
    ...
    return log.timeEvent(OperationType.CALCULATE_METRICS, () => {
      ...
      growthPaths.forEach((paths, growthNodeIndex) => {
        ...
        rv.push(
          new LeakRoot(growthNodeIndex, paths.map(edgePathToPath), {
            retainedSize,
            leakShare,
...
export class HeapGraph {
  public static async Construct(
    ...
    return log.timeEvent(OperationType.HEAP_SNAPSHOT_PARSE, async () => {
      ...
      const rootNodeIndex = (
        snapshotInfo.root_index ? snapshotInfo.root_index / nodeLength : 0
      ) as NodeIndex;
...

      {
          ...
          switch (chunk.type) {
            case DataTypes.NODES: {
              ...
              if (dataLen % nodeLength !== 0) {
                throw new Error(
                  `Expected chunk to contain whole nodes. Instead, contained ${dataNodeCount} nodes.`
                );
              }
            ...
            case DataTypes.EDGES: {
              ...
              if (dataLen % edgeLength !== 0) {
                throw new Error(
                  `Expected chunk to contain whole nodes. Instead, contained ${dataEdgeCount} nodes.`
                );
              }
              // Copy data into our typed arrays.
              for (let i = 0; i < dataEdgeCount; i++) {
                ...
                edgeNamesOrIndexes[arrayBase] =
                  data[dataBase + edgeNameOrIndexOffset];
                edgeToNodes[arrayBase] =
                  data[dataBase + edgeToNodeOffset] / nodeLength;
              }
        ...
        for (let i = 0; i < edgeCount; i++) {
          ...
          switch (edgeType) {
            ...
            case SnapshotEdgeType.Weak: // Weak reference: Doesn't hold onto memory.
            case SnapshotEdgeType.Property: // Property on an object.
              edgeNamesOrIndexes[i] = stringMap.get(
                strings[edgeNamesOrIndexes[i]]
              );
              break;
            default:
  ...
  private constructor(
    stringMap: StringMap,
    nodeTypes: Uint8Array,
    nodeNames: Uint32Array,
    nodeSizes: Uint32Array,
    firstEdgeIndexes: Uint32Array,
    edgeTypes: Uint8Array,
    edgeNamesOrIndexes: Uint32Array,
    edgeToNodes: Uint32Array,
  ...
  public getUserRootIndices(): number[] {
    ...
    for (const it = root.children; it.hasNext(); it.next()) {
      const subroot = it.item().to;
      if (
        subroot.type !== SnapshotNodeType.Synthetic ||
        subroot.name === "(Document DOM trees)"
      ) {
        rv.push(subroot.nodeIndex);
      }
    }
  ...
  public visitRoot(
    visitor: (n: Node) => void,
    filter: (n: Node, e: Edge) => boolean = nonWeakFilter
  ): void {
    bfsVisitor(this, [this.rootNodeIndex], visitor, filter);
  }

  public visitUserRoots(
    visitor: (n: Node) => void,
    filter: (n: Node, e: Edge) => boolean = nonWeakFilter
  ) {
  ...
  public visitGlobalEdges(
    visitor: (e: Edge, getPath: () => Edge[]) => void,
    filter: (n: Node, e: Edge) => boolean = nonWeakFilter
  ): void {
 ...
 */
function bfsEdgeVisitor(
  g: HeapGraph,
  initial: number[],
  visitor: (e: Edge, getPath: () => Edge[]) => void,
  filter: (n: Node, e: Edge) => boolean = nopFilter
): void {
...
function bfsVisitor(
  ...
  const nodesToVisit: { [n: number]: NodeIndex } & Uint32Array = <any>(
    new Uint32Array(g.nodeCount)
  );
  ...
  let index = 0;
  initial.map((idx: number) => enqueue(idx as NodeIndex));
