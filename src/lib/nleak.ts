...
import {
  FindAndDiagnoseLeaks,
  EvaluateRankingMetricsOperation,
  OperationState,
} from "./operations";
...
export class BLeakDetector {
   ...
   */
  public static async FindLeaks(
    configSource: string,
    progressBar: IProgressBar,
    driver: NodeDriver,
    flushResults: (results: BLeakResults) => void,
    snapshotCb: (sn: HeapSnapshotParser) => Promise<void> = defaultSnapshotCb,
    bleakResults?: BLeakResults
  ): Promise<BLeakResults> {
    const detector = new BLeakDetector(
      driver,
      progressBar,
      configSource,
      snapshotCb
   ...
   */
  public static async EvaluateRankingMetrics(
    configSource: string,
    progressBar: IProgressBar,
    driver: NodeDriver,
    ...
    snapshotCb: (
      ...
      leaksFixed: number,
      iteration: number
    ) => Promise<void> = defaultSnapshotCb
  ): Promise<void> {
   ...
   * BLeak algorithm.
   */
  public async findAndDiagnoseLeaks(
    flushResults: (results: BLeakResults) => void,
    bleakResults?: BLeakResults
  ): Promise<BLeakResults> {
    const op = new FindAndDiagnoseLeaks(
      this._config,
      flushResults,
      this._snapshotCb
    );
    this._progressBar.setOperationCount(op.size());
    const os = new OperationState(
      this._driver,
      this._progressBar,
      this._config
    );
   ...
   */
  public async evaluateRankingMetrics(
    results: BLeakResults,
    flushResults: (results: BLeakResults) => void,
    snapshotCb: (
    ...
    const op = new EvaluateRankingMetricsOperation(
      this._config,
      results,
      flushResults,
      snapshotCb
